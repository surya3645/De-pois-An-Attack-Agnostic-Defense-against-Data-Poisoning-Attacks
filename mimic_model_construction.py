...
class CWGANGP():
    ...
    def train(self, will_load_model):
        ...
        X_train = np.load("/content/saved_TrueAndGeneratorData.npy")  
        y_train = np.load("/content/saved_TrueAndGeneratorLabel.npy") 
    ...
    def sample_images(self, epoch):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig("/content/drive/MyDrive/MAIN/images_cwgan/mnist_%d.png" % (epoch))
        plt.close()
    ...
    def generate_images(self, label):
        ...
        self.generator.load_weights('weights/generator_CWGANGP_%d'%self.epochs)
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig("/content/drive/MyDrive/MAIN/images_generated/%d.png" %self.epochs)
