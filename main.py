...
def load_data():
    path = '/content/drive/MyDrive/MAIN/ds-praticuum-20220408T190328Z-001/ds-praticuum/De-Pois-Code/mnist.npz/mnist.npz'
    f = np.load(path)
...
def TrueAndGeneratorData(know_rate, epochs):
    ...
    if (os.path.exists("Generator_data_%d_%d.npy"%(generator_size, epochs))):
        ...
        G_data = np.load("/content/Generator_data_%d_%d.npy"%(generator_size, epochs))
...
def poi_data(poison_rate):   
    ...
    poisoned_x_data = genfromtxt("/content/p_images.csv")
    poisoned_y_data = genfromtxt("/content/p_labels.csv")
